        ...
        *******************/
        ...
        $scope.printBarcode = function( divId ){
            ...
            if( 'undefined' != typeof $scope.item.serial_number ){
                barcodeWindow.document.write('<tr><td>Número de serie</td><td>' + $scope.item.serial_number + '</td></tr>');
            }

            if( $scope.item.parts ){
                $.each($scope.item.parts, function(i, val){
                    barcodeWindow.document.write('<tr><td>Número de serie ' + val.name + '</td><td>' + val.serial_number + '</td></tr>');
                });
            }

            if( 'undefined' != typeof $scope.item.locations ){
                var locations = '';
                $.each( $scope.item.locations, function( i, val ){
                    locations += val.location + ' ,';
                })
                barcodeWindow.document.write('<tr><td>Ubicaciones</td><td>' + locations.slice(0, -1) + '</td></tr>');
            }
